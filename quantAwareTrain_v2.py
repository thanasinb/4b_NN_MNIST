...
def trainQuantAware(args, model, device, train_loader, optimizer, epoch, stats, act_quant=False, num_bits=8, mnist=True):
    model.train()
    correct = 0
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if (mnist and batch_idx == 999) or (not mnist and batch_idx == 499):
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}, Accuracy: {}/{} ({:.0f}%), batch_idx: {}, log_interval: {}'.format(
